...
{
    uint8_t rx_byte = 0;
    (void)uwb_platform_spi_transfer(&tx_byte, &rx_byte, 1U);
    return rx_byte;
}

static void SPI_TransferBytes(const uint8_t *tx, uint8_t *rx, uint16_t len)
{
    (void)uwb_platform_spi_transfer(tx, rx, len);
}
...
void DW1000_Configure(void)
{
     ...
     * by a software timeout using Zephyr uptime.
 ...
 *         (dùng MCU_ElapsedUs() cho mục đích đó — FIX-02).
 */
...
/**
 * @brief  Đọc mức chân IRQ (PA3) — 1 nếu còn event chờ xử lý.
 */
