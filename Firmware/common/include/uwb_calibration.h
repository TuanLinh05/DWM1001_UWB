 ...
 *
 * NGUYÊN TẮC AN TOÀN: cấu hình board quyết định compensation profile. Mọi
 * calibration mask mặc định fail-closed cho tới khi phần cứng thật được đo.
 *
...
/* ========================================================================== */
/*                  FEATURE FLAGS (mặc định an toàn cho PCB mới)               */
/* ========================================================================== */

/** 1: write antenna delay into the DW1000; 0: legacy software compensation. */
#ifndef UWB_USE_HW_ANTENNA_DELAY
#define UWB_USE_HW_ANTENNA_DELAY   1
#endif

/** 1: legacy large software offset; 0: small residual after HW antenna delay. */
...
/** 1: DS-TWR 4-message (POLL/RESP/FINAL/REPORT); 0: SS-TWR 2-message. */
 ...
 *  Bit 0..7 tương ứng A1..A8. Chỉ set một bit sau khi đúng anchor đó
...
#define UWB_DS_CAL_A5_BIT          (1U << 4)
#define UWB_DS_CAL_A6_BIT          (1U << 5)
#define UWB_DS_CAL_A7_BIT          (1U << 6)
#define UWB_DS_CAL_A8_BIT          (1U << 7)
...
#define UWB_SS_CAL_A5_BIT          (1U << 4)
#define UWB_SS_CAL_A6_BIT          (1U << 5)
#define UWB_SS_CAL_A7_BIT          (1U << 6)
#define UWB_SS_CAL_A8_BIT          (1U << 7)
...
/* OFF by default: the estimator now runs on the host (ADR-001) and SHADOW
 * doubled the soft-float filter work inside every ranging slot. */
#ifndef UWB_LEGACY_ADAPTIVE_MODE
#define UWB_LEGACY_ADAPTIVE_MODE UWB_LEGACY_ADAPTIVE_OFF
#endif
...
/** Số outlier liên tiếp trước khi snap về giá trị đo (backstop).
 *  15 × 20ms = 0,3s ở chu kỳ 50 Hz. */
...
/* ========================================================================== */
/*          RADIO TUNING (GĐ2) — change only with a hardware A/B              */
/* ========================================================================== */

/**
 * TX power profile. Every profile other than LEGACY shifts range bias and
 * therefore invalidates any calibration captured before the change.
 *   LEGACY    : 0x1E1E1E1E, smart TX power off — deployed baseline (~30 dB
 *               gain, about 16 dB above the User Manual reference).
 *   REFERENCE : 0x48484848, smart TX power off — DW1000 UM reference, Ch5 PRF16.
 *   SMART     : 0x0E082848, smart TX power on  — DW1000 UM reference, Ch5 PRF16.
 *   CUSTOM    : UWB_TX_POWER_CUSTOM_VALUE, smart TX power off.
 * The TAG can also switch profile at runtime through a command (uwb_cmd.h).
 */
#define UWB_TX_POWER_LEGACY     0U
#define UWB_TX_POWER_REFERENCE  1U
#define UWB_TX_POWER_SMART      2U
#define UWB_TX_POWER_CUSTOM     3U

#ifndef UWB_TX_POWER_MODE
#define UWB_TX_POWER_MODE       UWB_TX_POWER_LEGACY
#endif
#ifndef UWB_TX_POWER_CUSTOM_VALUE
#define UWB_TX_POWER_CUSTOM_VALUE 0x48484848UL
#endif
#if UWB_TX_POWER_MODE > UWB_TX_POWER_CUSTOM
#error "Unknown UWB_TX_POWER_MODE"
#endif

/**
 * 1: apply the DW1000 reference tuning the baseline skipped — LDE NTM = 13
 * (LDE_CFG1 = 0x6D), LDOTUNE kick when OTP holds a value and crystal trim
 * from OTP (mid-range 0x10 when the OTP word is blank). OTP is always read
 * and reported; this flag only controls whether the values are applied.
 */
#ifndef UWB_DW_REFERENCE_TUNING
#define UWB_DW_REFERENCE_TUNING 0
#endif
#if (UWB_DW_REFERENCE_TUNING != 0) && (UWB_DW_REFERENCE_TUNING != 1)
#error "UWB_DW_REFERENCE_TUNING must be 0 or 1"
#endif
...
/** Exact DW1000 device-time unit: 1 / (499.2 MHz * 128). */
#define UWB_DWT_TIME_UNIT_S   (1.0 / (499.2e6 * 128.0))

/** Propagation speed in air used by the Decawave ranging examples (m/s). */
#define UWB_SPEED_OF_LIGHT    299702547.0

/** Carrier integrator → clock offset ratio (Ch5, 6.8 Mbps).
 *  Decawave reference (ex_06a SS-TWR initiator, deca_device_api.h):
 *    ratio = CI × FREQ_OFFSET_MULTIPLIER × HERTZ_TO_PPM_MULTIPLIER_CHAN_5 / 1e6
 *          = CI × (998.4e6 / 2 / 1024 / 131072) × (−1e6 / 6489.6e6) / 1e6
 *          = CI × −5.7312e−10,
 *  applied as T_reply × (1 − ratio). The baseline used +5.7312e−10, which
 *  doubles the SS error instead of removing it. Verify on hardware before
 *  enabling UWB_USE_CLOCK_CORRECTION: SS-with-correction must match DS. */
...
/** Scale offset applied to FPP before it reaches the TAG-side filters.
 *  Their thresholds (kalman_measurement_noise, reacquire FPP) were tuned on
 *  hardware logs recorded while RXPACC was mis-read, which reported FPP
 *  20·log10(4) ≈ 12.04 dB too low. The filters keep that scale so their
 *  validated behaviour does not change; telemetry carries the corrected FPP.
 *  Set to 0.0f once the filters are retuned on corrected FPP. */
...
#endif
#ifndef UWB_DS_OFFSET_A5_M
#define UWB_DS_OFFSET_A5_M   0.0
#endif
#ifndef UWB_DS_OFFSET_A6_M
#define UWB_DS_OFFSET_A6_M   0.0
#endif
#ifndef UWB_DS_OFFSET_A7_M
#define UWB_DS_OFFSET_A7_M   0.0
#endif
#ifndef UWB_DS_OFFSET_A8_M
