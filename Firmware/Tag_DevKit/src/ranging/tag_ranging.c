 ...
 ******************************************************************************
 * @file    tag_ranging.c
 * @brief   TAG ranging — interrupt-driven state machine @ 50Hz / 8 anchors
 *
 * Protocol: Single-Sided Two-Way Ranging (SS-TWR)
 *   For each Anchor (1 through 8):
 *     1. TAG  ──[POLL]──>  Anchor
 ...
 *   TAG_STATE_IDLE
 *     │  (every 20ms: uptime - last_cycle >= TAG_CYCLE_MS)
 *     ▼
 ...
 *
 * Key design:
 *   - No blocking delay in the ranging state machine.
...
/* SS-TWR legacy offsets. A1-A3 are validated; A4-A8 are deliberately left at
...
/* Keep the activity LED human-visible even when TAG ranges at tens or
 * hundreds of operations per second.  Activity means the UWB exchange and
 * ToF calculation succeeded; calibration validity is reported separately. */
#define TAG_ACTIVITY_LED_TOGGLE_INTERVAL_MS 100U

static uint32_t s_activity_led_last_toggle_ms;

static void note_successful_ranging_activity(void)
{
    uint32_t now_ms = uwb_platform_time_ms();

    if ((uint32_t)(now_ms - s_activity_led_last_toggle_ms) >=
        TAG_ACTIVITY_LED_TOGGLE_INTERVAL_MS)
    {
        LED_TOGGLE();
        s_activity_led_last_toggle_ms = now_ms;
    }
}
...
static const TagAnchorConfig_t s_anchor_cfg[TAG_NUM_ANCHORS] = {
    ...
    { 0x0005U, UWB_SS_CAL_A5_BIT },
    { 0x0006U, UWB_SS_CAL_A6_BIT },
    { 0x0007U, UWB_SS_CAL_A7_BIT },
    { 0x0008U, UWB_SS_CAL_A8_BIT },
};
                                               ...
                                               double *offset_m)
{
    ...
    switch (anchor_id_at(anchor_index))
    {
        ...
        case 0x0005U:
            calibration_bit = UWB_DS_CAL_A5_BIT;
            *offset_m = UWB_DS_OFFSET_A5_M;
            break;

        case 0x0006U:
            calibration_bit = UWB_DS_CAL_A6_BIT;
            *offset_m = UWB_DS_OFFSET_A6_M;
            break;

        case 0x0007U:
            calibration_bit = UWB_DS_CAL_A7_BIT;
            *offset_m = UWB_DS_OFFSET_A7_M;
            break;

        case 0x0008U:
            calibration_bit = UWB_DS_CAL_A8_BIT;
            *offset_m = UWB_DS_OFFSET_A8_M;
            break;
                                             ...
                                             int32_t raw_mm, float fpp_dbm)
{
    ...
    response_ok_count++;
    note_successful_ranging_activity();
 ...
 */
static __attribute__((unused)) int32_t compute_distance_mm(
    const uint8_t *resp_rx_ts_raw, const uint8_t *resp_frame,
    int32_t *uncalibrated_raw_mm)
{
                             ...
                             float fpp, uint8_t st_flags)
{
...
#if UWB_C9_2_MOTION_MODE == UWB_C9_2_MOTION_ACTIVE
    {
        C9_2MotionProcessResult_t motion_result = c9_2_motion_process(
            s_current_anchor, dist_mm, fpp_report, uwb_platform_time_ms());
    ...
    (void)legacy_adaptive_process(
        ...
        dist_mm, fpp_report, uwb_platform_time_ms());
...
void Tag_Task(void)
{
    ...
    switch (s_state)
    {
        ...
        /* ================================================================== */
        {
            ...
            if (dw1000_irq_flag)
            {
                ...
                else
                {
                    ...
                    if (rx_len != 16U || rx_buf[0] != 0x41U || rx_buf[1] != 0x88U
                        || rx_buf[3] != (uint8_t)DW_PAN_ID
                        || rx_buf[4] != (uint8_t)(DW_PAN_ID >> 8))
                    {
        ...
        /* ================================================================== */
        {
            ...
            {
                dw1000_irq_flag = 0;
                uint64_t status_rpt = DW1000_ReadStatus();
                ...
                else
                {
                    ...
                    if (rx_len_r == 16U && rx_buf_r[0] == 0x41U && rx_buf_r[1] == 0x88U
                        && rx_buf_r[3] == (uint8_t)DW_PAN_ID
                        && rx_buf_r[4] == (uint8_t)(DW_PAN_ID >> 8))
                    {
