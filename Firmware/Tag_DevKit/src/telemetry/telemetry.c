...
#define TELEM_UART_BAUD           115200U /* Must match app.overlay. */
...
_Static_assert(
    (TELEM_MAX_PKT * TELEM_CYCLE_HZ * TELEM_UART_BITS_PER_BYTE)
        <= ((TELEM_UART_BAUD * 8U) / 10U),
    "Range telemetry exceeds 80 percent of the 115200-baud UART budget");
...
void Telem_SendInfo(void)
{
    ...
    for (uint8_t i = 0U; i < TAG_NUM_ANCHORS && n > 0 && n < (int)sizeof(line); i++)
    {
        ...
        static const double ds_offset_m[TAG_NUM_ANCHORS] = {
            ...
            UWB_DS_OFFSET_A5_M, UWB_DS_OFFSET_A6_M,
            UWB_DS_OFFSET_A7_M, UWB_DS_OFFSET_A8_M
        };
    ...
    uint16_t total = frame_pack(buf, TELEM_TYPE_INFO,
                                tag_sample_seq, uwb_platform_time_ms(), payload_len);
