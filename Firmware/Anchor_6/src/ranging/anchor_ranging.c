 ...
 * = 0 khi UWB_USE_HW_ANTENNA_DELAY=0 (chỉ dùng cho profile legacy) → t_reply KHÔNG
...
static uint32_t             s_state_tick = 0;   /* Zephyr uptime at state entry */
...
void Anchor_Task(void)
{
    ...
    switch (s_state)
    {
        ...
        /* ============================================================== */
        {
            ...
            if (rx_len != 12U || rx_buf[0] != 0x41U || rx_buf[1] != 0x88U
                || rx_buf[3] != (uint8_t)DW_PAN_ID
                || rx_buf[4] != (uint8_t)(DW_PAN_ID >> 8)
                || rx_buf[7] != (uint8_t)TAG_ADDR
                || rx_buf[8] != (uint8_t)(TAG_ADDR >> 8))
            {
                reset_to_rx_wait();
                break;
            }
