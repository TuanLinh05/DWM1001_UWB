...
function Initialize-NcsEnvironment {
    ...
    return [pscustomobject]@{
        ...
        West = $west
        Gdb = Join-Path $toolchain 'opt\zephyr-sdk\gnu\arm-zephyr-eabi\bin\arm-zephyr-eabi-gdb.exe'
    }
}

function Find-OpenOcdEnvironment {
    param(
    ...
    $executables = @(@(
        $RequestedExecutable,
        $env:OPENOCD_PATH,
        'E:\software\OpenOCD\bin\openocd.exe'
    ) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })

    $arduinoRoot = Join-Path $env:LOCALAPPDATA 'Arduino15\packages\esp32\tools\openocd-esp32'
    if (Test-Path -LiteralPath $arduinoRoot) {
        $executables += Get-ChildItem -LiteralPath $arduinoRoot -Directory |
            Sort-Object LastWriteTime -Descending |
            ForEach-Object { Join-Path $_.FullName 'bin\openocd.exe' }
    }

    $uniqueExecutables = @($executables | Select-Object -Unique)
    foreach ($executable in $uniqueExecutables) {
        if (-not (Test-Path -LiteralPath $executable)) {
            continue
        }

        $openOcdRoot = Split-Path (Split-Path $executable -Parent) -Parent
        $scriptCandidates = @(
            $RequestedScripts,
            $env:OPENOCD_SCRIPTS,
            (Join-Path $openOcdRoot 'share\openocd\scripts')
        ) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }

        foreach ($scripts in $scriptCandidates) {
            if ((Test-Path -LiteralPath (Join-Path $scripts 'interface\stlink.cfg')) -and
                (Test-Path -LiteralPath (Join-Path $scripts 'target\nordic\nrf52.cfg'))) {
                return [pscustomobject]@{
    ...
    throw 'OpenOCD with ST-LINK and Nordic nRF52 scripts was not found. Set OPENOCD_PATH and OPENOCD_SCRIPTS.'
}
