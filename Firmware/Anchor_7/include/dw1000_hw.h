...
/* ANCHOR_ADDR and TAG_ADDR are selected by Kconfig in uwb_app_config.h. */
...
/**
 * @brief Signal quality diagnostics for Kalman filter adaptive tuning
 */
typedef struct {
 ...
 *         Preamble 256, PAC16 and 6.8 Mbps.
 */
 ...
 *         ngay HPDWARN/TXPUTE — nếu thời điểm phát đã trôi qua hoặc DW1000
 *         không kịp power-up, TX sẽ không xảy ra.
 * @retval 0  = đã lên lịch TX thành công
 * @retval -1 = TRỄ/không kịp power-up — caller recovery ngay, không chờ timeout
 */
...
/**
 * @brief  Read the signal quality diagnostics (RXPACC, FP_AMPL1-3)
 * @param  diag  Pointer to struct to store the diagnostics
 */
void DW1000_ReadSignalDiag(DW1000_SignalDiag_t* diag);

/**
 * @brief  Calculate the First Path Power (FPP) in dBm from diagnostics
 * @param  diag  Pointer to populated diagnostics struct
 * @retval FPP in dBm
 */
float DW1000_GetFirstPathPower(const DW1000_SignalDiag_t* diag);

/**
 * @brief  Read the Carrier Integrator (Frequency offset)
 * @retval 21-bit signed carrier integrator value
 */
int32_t DW1000_ReadCarrierIntegrator(void);

/**
 * @brief  Clear all status flags by writing ones to all 5 SYS_STATUS bytes.
 */
...
/**
 * @brief  Configure the devicetree DW1000 IRQ pin as a rising-edge interrupt.
 *         Must be called AFTER DW1000_Init() +
 *         DW1000_Configure() + DW1000_ClearAllStatus().
 *
 * When DW1000 signals TX done or RX done, its IRQ remains HIGH until status
 * is cleared. The Zephyr GPIO callback sets dw1000_irq_flag = 1.
 * Application code must poll this flag and clear it.
 */
void DW1000_EnableIRQ(void);

/**
 * @brief  IRQ flag set by the Zephyr GPIO callback.
...
/* Transport failures since boot; inspect when the application enters fatal blink. */
...
/**
 * @brief  Micro-giây kể từ MCU_TimerInit() (wrap ~67s @64MHz).
...
/**
 * @brief  FIX-02: kiểu dữ liệu "mốc cycle thô" — dùng làm điểm bắt đầu cho
 *         MCU_ElapsedUs(). KHÔNG phải microsecond, chỉ là snapshot CYCCNT thô.
 */
typedef uint32_t McuCycleStamp_t;

/**
 * @brief  FIX-02: Đọc mốc cycle thô hiện tại (CYCCNT). Dùng làm `start` cho
 *         MCU_ElapsedUs() thay vì lưu MCU_Micros() để tính delta timeout.
 */
McuCycleStamp_t MCU_CycleNow(void);

/**
 * @brief  FIX-02: Micro-giây đã trôi qua kể từ mốc `start` (MCU_CycleNow()).
 *         An toàn qua điểm wrap CYCCNT (~67s @64MHz) vì trừ RAW CYCCNT
 *         (uint32_t, tự động modulo 2^32 đúng chuẩn) TRƯỚC khi quy đổi sang µs.
 *         Khác MCU_Micros(): hàm đó quy đổi trước rồi nơi gọi mới trừ — SAI
 *         qua điểm wrap (xem FIX-02 trong FIRMWARE_CORRECTNESS_FIXES).
 */
uint32_t MCU_ElapsedUs(McuCycleStamp_t start);

/**
 * @brief  Read the active level of the DW1000 IRQ devicetree pin.
 * @retval 1 nếu IRQ đang HIGH (còn event chưa xử lý), 0 nếu LOW.
 *
 * IRQ của DW1000 là tín hiệu MỨC (giữ HIGH tới khi clear status). EXTI cạnh lên
 * có thể lỡ event thứ hai; poll mức này để bù (bổ sung F trong review).
 */
uint8_t DW1000_IrqLineActive(void);

/**
 * @brief  Đọc lại các thanh ghi PHY quan trọng và so với giá trị kỳ vọng.
 * @retval Bitmask các thanh ghi lệch (0 = tất cả khớp). Cho debug/telemetry (F-03).
 */
uint32_t DW1000_VerifyConfig(void);

/**
 * @brief  Đọc cảm biến nhiệt độ die của DW1000 (chuỗi SAR, ~1ms, BLOCKING).
 *         Chỉ gọi khi radio IDLE (vd lúc khởi động) — nó ghi tạm RF_CONF bias.
 * @retval Giá trị raw nhiệt độ (byte SAR). Xem Phase 3 plan mục 6 để đổi ra °C.
 */
